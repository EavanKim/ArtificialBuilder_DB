...
namespace ArtificialBuilder.DB
{
    ...
    // storage-policy 2026-05-17 § "시계열 샤딩 DB 스키마" 정합. AB_Object_DB_Turn_Row 단일 DbSet.
    // round 1 (skeleton) = DbSet + OnModelCreating PK + index 만.
    internal sealed class AB_Context_DB_Turn_Shard : DbContext
    {
        ...
        {
            // PK = TurnId.
            // index = PrevTurnId / BranchRootTurnId — 시계열 traverse / refresh 갈래 query 매개 (round 2 query 본체 결재).
            // circuit_id 컬럼 X — 폴더명 매개 식별.
            _builder.Entity<AB_Object_DB_Turn_Row>(_e =>
            {
                _e.HasKey(_x => _x.TurnId);
                _e.HasIndex(_x => _x.PrevTurnId);
                _e.HasIndex(_x => _x.BranchRootTurnId);
            });
        }
    }
}
